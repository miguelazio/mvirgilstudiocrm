...
const simpleModal = `function ProjectTypesModal({ t, lang, dbProjectTypes, dbProjectPhases, onReload, reloadPhases, setModal, onClose }) {
  ...
  const seedPhases = async () => {
    alert("Attempting to seed phases...");
    setLoading(true);
    try {
      let seededCount = 0;
      for (const type of types) {
        ...
        if (existing.length === 0 && SEEDED_PHASES[type.key]) {
          ...
          if (error) {
            alert("Error with " + type.key + ": " + error.message);
          } else {
            seededCount++;
          }
        }
      }
      alert("Done! Seeded " + seededCount + " types.");
...
if (startIndex !== -1) {
  content = content.substring(0, startIndex) + simpleModal;
  fs.writeFileSync(path, content, 'utf8');
  console.log('Fixed ProjectTypesModal with simple version');
}
`;

fs.writeFileSync('c:\\Users\\corsair\\Desktop\\vibecoding\\materiaVolume_CRM_claude_v01\\simple_modal_fix.cjs', simpleModalScript, 'utf8');

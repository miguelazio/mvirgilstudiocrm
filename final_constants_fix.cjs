...
if (!content.includes('const DEFAULT_PROJECT_TYPES')) {
  const pos = content.indexOf('export default function App()');
  if (pos !== -1) {
    content = content.slice(0, pos) + constantsDef + content.slice(pos);
    fs.writeFileSync('src/App.jsx', content);
    console.log('Fixed: Constants added before App function.');
  } else {
    console.log('Error: Could not find App function.');
  }
} else {
  console.log('Already there: Constants already exist.');
}
